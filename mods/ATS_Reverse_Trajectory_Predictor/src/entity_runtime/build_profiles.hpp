...
// ATS 1.61.2.0 Steam public executable (2026-09-25 install). The executable
// hash selects this profile exactly; all four entry hooks and four runtime
// helpers are checked before the native runtime is enabled.
constexpr std::array verified_ats_161_2_0_hooks{
    HookSpec{HookId::model_load, "model_load", 0x015dacf0,
             {0x40, 0x53, 0x56, 0x41, 0x56, 0x48, 0x83, 0xec,
              0x40, 0x80, 0xbc, 0x24, 0x80, 0x00, 0x00, 0x00}},
    HookSpec{HookId::vehicle_render_dispatch, "vehicle_render_dispatch",
             0x00799b20,
    ...
    HookSpec{HookId::trailer_visual_update, "trailer_visual_update",
             ...
             {0x48, 0x8b, 0xc4, 0x48, 0x89, 0x70, 0x10, 0x48,
              0x89, 0x78, 0x18, 0x55, 0x48, 0x8d, 0x68, 0xa1}},
    HookSpec{HookId::trailer_render, "trailer_render", 0x00639190,
             {0x48, 0x89, 0x5c, 0x24, 0x08, 0x48, 0x89, 0x74,
              0x24, 0x10, 0x57, 0x48, 0x83, 0xec, 0x30, 0x49}},
};

constexpr RuntimeRvas verified_ats_161_2_0_runtime{
    0x00000000, // model_activate intentionally not called on 1.61
    0x0033de80, // model_transfer
    0x0041df90, // model_parameter_init
    0x00649a10, // vehicle_accessory_collect (unused by model_load_hook_only)
    0x0064a250, // vehicle_addon_finalize (unused by model_load_hook_only)
    0x00312e60, // render_entry_populate (unused by model_load_hook_only)
    0x00000000, // final_base_model_create (lifecycle hook disabled)
    0x00000000, // final_base_model_create_return
    0x01517fa0, // final_model_create (unused by model_load_hook_only)
    0x00000000, // final_model_create_return (unused)
    0x00000000, // final_accessory_insert (unused)
    0x00000000, // final_accessory_insert_return
    0x013cda50, // set_parent
    0x013cdb10, // set_transform
};

constexpr std::array verified_ats_161_2_0_runtime_signatures{
    RuntimeSignatureSpec{"model_transfer", 0x0033de80,
                         {0x48, 0x89, 0x5c, 0x24, 0x08, 0x48, 0x89, 0x74,
                          0x24, 0x10, 0x57, 0x48, 0x83, 0xec, 0x20, 0x48}},
    RuntimeSignatureSpec{"model_parameter_init", 0x0041df90,
                         {0x48, 0x89, 0x5c, 0x24, 0x08, 0x48, 0x89, 0x6c,
                          0x24, 0x18, 0x56, 0x57, 0x41, 0x56, 0x48, 0x83}},
    RuntimeSignatureSpec{"set_parent", 0x013cda50,
                         {0x48, 0x83, 0xec, 0x28, 0x4c, 0x8b, 0xc1, 0x4c,
                          0x8b, 0xca, 0x48, 0x8b, 0x49, 0x40, 0x48, 0x3b}},
    RuntimeSignatureSpec{"set_transform", 0x013cdb10,
                         {0x48, 0x89, 0x5c, 0x24, 0x08, 0x57, 0x48, 0x83,
                          0xec, 0x40, 0x48, 0x8b, 0xd9, 0x48, 0x8b, 0xfa}},
};

// Exact public ATS builds are hash-gated. The final profile deliberately has no
// hash: it admits another ATS 1.60 binary only when every enabled hook has the
// exact verified RVA and bytes. Unused lifecycle helpers are not compatibility
// gates.
constexpr std::array build_profiles{
    ...
    BuildProfile{
        "ats-1.61.2.0-steam-public",
        "ATS 1.61.2.0",
        "9DF9745ABEE9C698919B572A1E6C645C00E4F37218A90498589C0101A391740C",
        false,
        verified_ats_161_2_0_hooks,
        verified_ats_161_2_0_runtime,
        verified_ats_161_2_0_runtime_signatures,
    },
    BuildProfile{
